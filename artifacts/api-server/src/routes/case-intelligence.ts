  ...
  try {
    ...
    const workers = await query<{ id: string }>(
      `SELECT DISTINCT w.id FROM workers w
       LEFT JOIN legal_cases lc ON lc.worker_id = w.id AND lc.tenant_id = $1
    ...
    for (const { id } of workers) {
      try {
        ...
        criticalRisks += r.risks.filter(rk => rk.severity === "CRITICAL").length;
        if (r.nextActions[0]) topActions.push({ workerId: id, workerName: r.workerName, action: r.nextActions[0].action, priority: r.nextActions[0].priority });
        breakdown.push({ workerId: id, workerName: r.workerName, readiness: r.readiness, completeness: r.completenessScore, riskLevel: r.overallRiskLevel, topAction: r.nextActions[0]?.action ?? "None" });
      } catch { /* skip */ }
    }
    ...
    res.json({
      batch: { totalWorkers: workers.length, notReady, inProgress, readyForSubmission, criticalRisks, topActions: topActions.slice(0, 20), workerBreakdown: breakdown.slice(0, 50), computedAt: new Date().toISOString() },
    });
  } catch (err) {
