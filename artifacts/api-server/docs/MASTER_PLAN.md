...
Polish authoritative, English bridge. The system operates with Polish as the primary and authoritative language because Polish law, Polish authorities, and Polish administrative practice are the regulatory ground truth. AI outputs render in Polish by default on all result pages (pages opened by buttons where AI-generated content, authority correspondence, or other result content displays to the user). English translation is available on every result page via per-page toggle, accessible in zero or one clicks, providing a comprehension bridge for clients (foreign workers who may not read Polish well) and for system operators. Where translations diverge, Polish prevails. Implementation status: principle is constitutional; implementation is in two-tier remediation per LANGUAGE_TOGGLE_VERIFICATION.md (commit 41dedd1). Tier 1 (toggle availability + foundational defaults across all result pages) targets ~4-8 hours of focused work. Tier 2 (per-page content translation across 106 pages currently without i18n) is a multi-month sub-phase. Until Tier 2 lands per page, that page may render English even when Polish is selected.
