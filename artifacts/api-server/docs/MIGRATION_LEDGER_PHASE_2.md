...
| AC-55 | **APATRIS Communication System** - Refined scope per 3-phase deep research (commits 3bec12e + 9de9371 + 8d2e1cd): 5 categories - (a) Transport spine (hybrid Push+WebSocket per industry standard - APATRIS push + EEJ WebSocket cross-pollinate), (b) AI gen + approval (3-tier UX + audit trail per regulated-industry canon - both sides need UI surface), (c) Preference center (per-user × per-channel × per-message-type per Knock/Courier/MagicBell convergence - both sides absent), (d) Regulatory scaffolding (handled separately at AC-58 - this AC integrates compliance hooks), (e) Moderation (in-house Claude-based, defer third-party APIs). Silence-watcher capability DROPPED per Phase 3 finding (no industry precedent + GDPR Art 22 risk if AI auto-replies without human review). Principle per Manish Day 30 evening: APATRIS owns the channel - WhatsApp messages can be lost, not audited, not automatable cleanly. APATRIS chat = full control + audit + automation. Type: NEW BUILD (umbrella) - likely reframes AC-40 + AC-43 + AC-44 + AC-46 into coherent system rather than parallel features. Upstream gate: AC-58 compliance scaffolding decisions; Yulia legal-input session. Source: Manish Day 30 evening prompt; Day 31 reshape per 3-phase deep research; refined into 5 categories with industry validation + regulatory integration via AC-58. Phase A audit pending after AC-58 scaffolding decisions made. | M3 |
