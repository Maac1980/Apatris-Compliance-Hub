...
| AC-53 | **APATRIS Worker Cockpit - operator-domain-specific role-adaptive profile** - inspired by EEJ WorkerCockpit (11 panels, role-adaptive) but built around APATRIS's actual operator personas + modules. NOT a port. Role-adaptive panels for Yulia (legal-head: TRC case timeline / appointment management / document prep / work-permit status / A1 / posted-worker compliance), Akshay (ops-coordinator: site assignment / welder qualifications / Tekra/Izotechnik/Gaztech client allocation / BHP status / UDT certifications), Manish (owner-admin: cross-cutting strategic view). Type: NEW BUILD (APATRIS-specific design + EEJ as proof-of-concept reference only). Upstream gate: AC-49 RBAC role taxonomy + AC-35 Yulia + Akshay walkthrough Monday for panel scope validation. Source: APATRIS-vs-EEJ Feature Audit Day 30 (commit c4355d8) + Manish "better not copy" principle. | M3 |
| AC-54 | **APATRIS Deep-link primitive - worker-context navigation to APATRIS-specific modules** - same primitive pattern as EEJ setDeepLinkWorker + onOpenModule but routes to APATRIS module set (TRC / Work permit / A1 / Posted workers / UDT / ZUS-Płace / BHP / Contract amendments Aneks). NOT a port of EEJ's module names. Type: NEW BUILD (APATRIS-module-specific navigation). Upstream gate: AC-31 Phase B kickoff + AC-53 panel design defines target modules. Source: APATRIS-vs-EEJ Feature Audit Day 30 (commit c4355d8). | M3 |
