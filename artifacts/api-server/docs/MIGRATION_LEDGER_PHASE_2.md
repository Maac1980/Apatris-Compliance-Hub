...
| AC-55 | **APATRIS Communication System - in-app chat replacing WhatsApp as primary channel** - umbrella feature covering (a) in-app worker chat (worker <-> coordinator/admin, replaces WhatsApp as default), (b) broadcast one-to-many message + per-audience refine/translate (by site/role/tier/language), (c) AI auto-flag + auto-email to admin/coordinators when issues detected in messaging patterns or worker signals, (d) AI generic messages to workers when humans don't respond (fallback outreach for missed check-in / absence non-reply / overdue documents), (e) WhatsApp retained as fallback channel only (not removed). Principle per Manish Day 30 evening: APATRIS owns the channel - WhatsApp messages can be lost, not audited, not automatable cleanly. APATRIS chat = full control + audit + automation. Type: NEW BUILD (umbrella) - likely reframes AC-40 + AC-43 + AC-44 + AC-46 into coherent system rather than parallel features. Phase A audit scope (Day 31+): deep research = APATRIS comms infrastructure + EEJ comms patterns + web research on competitor workforce-comms (Beekeeper / Workvivo / Slack-for-workforce / Atria patterns / AI-augmented comms systems) + map 5 capabilities to existing vs net-new. Upstream gate: deep research audit completion. Source: Manish Day 30 evening prompt. | M3 |
