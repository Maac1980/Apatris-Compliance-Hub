...
| AC-39 | **Worker Leave tab improvements** — extend existing `LeaveTab.tsx` + `/api/self-service/leave` + `leave_requests` table with rule-driven cutoffs, balance display, lateness category. Type: EXTEND. Upstream gate: none. Phase A: pending. Phase B: pending. Source: Worker App Vision Scoping Audit Day 30; Day 31 regulatory zone update per AC-55 Phase 3 research (commit 8d2e1cd): AC-39 Wave 1 ship Day 30 (v304) is now in newly-regulated zone per Kodeks pracy January 27, 2026 amendments - electronic leave-request format requires integrity + identifiability + RODO compliance. Verification pending via AC-58 Phase A audit + Yulia legal-input session. AC-39 may need Wave 2 adjustments for regulatory alignment. | M3 |
| AC-40 | **Worker Help tab AI assistant + categorized issues** — extend existing `MessagingTab.tsx` (operator-worker threads) with AI assistant + categorized issue selection. Type: EXTEND. Upstream gate: none. Phase A: pending. Phase B: pending. Source: Worker App Vision Scoping Audit Day 30. — Day 31 update per GATE D31-WORKER-NAV-AUDIT: Wave 1 receiverId fix shipped at v305 is operationally inert - MessagingTab unreachable from any tier's bottom nav (file:line evidence: BottomNav.tsx zero 'messages' entries; DashboardPage.tsx:323-324 route exists but no UI surface exposes it). AC-56 captures the nav exposure work needed to operationalize AC-40 Wave 1. AC-40 fix is functionally correct + ready to serve users once AC-56 ships. | M3 |
...
| AC-53 | **APATRIS Worker Profile - 3 AI output surfaces** - APATRIS's existing worker profile layout is operator-validated excellent (Manish Day 30 evening: "APATRIS layout is amazing and I love it"; "EEJ layout took hours to fix"). AC-53 scope is NARROW: add 3 specific AI-output surfaces operators have validated would be useful. (a) AI Summary narrative paragraph synthesizing worker compliance state, (b) AI Action Recommendations - 3-rec structure with reasoning, role-segmented per operator (Yulia legal / Akshay ops / Manish strategic), (c) AI Decisions audit log UI panel surfacing existing API-only audit trail. APATRIS layout intact. NOT a port, NOT a redesign. AC-53 is an AI feature AC, not a layout AC. Type: ENHANCE. Upstream gate: AC-44 internal AI orchestration Phase B Wave 1 (provides registry + handler architecture) + AC-35 operator interviews (establishes per-role rec content). Source: APATRIS-vs-EEJ Feature Audit Day 30 (commit c4355d8) + Manish operator correction commit 90dc87a evening Day 30. | M3 |
| AC-54 | **Deep-link primitive (worker-context navigation) - DEFERRED to Monday walkthrough** - APATRIS layout is operator-validated excellent and may already handle deep-link navigation in ways the cross-audit didn't detect (file-count audits never override operator-eyes per CLAUDE.md doctrine). Whether the gap is real, partial, or non-existent needs Monday text-dump walkthrough of APATRIS dashboard pages before scope locks. Phase A audit deferred. Source: APATRIS-vs-EEJ Feature Audit Day 30 (commit c4355d8) + Manish operator correction commit 90dc87a. | parked |
| AC-55 | **APATRIS Communication System** - Refined scope per 3-phase deep research (commits 3bec12e + 9de9371 + 8d2e1cd): 5 categories - (a) Transport spine (hybrid Push+WebSocket per industry standard - APATRIS push + EEJ WebSocket cross-pollinate), (b) AI gen + approval (3-tier UX + audit trail per regulated-industry canon - both sides need UI surface), (c) Preference center (per-user × per-channel × per-message-type per Knock/Courier/MagicBell convergence - both sides absent), (d) Regulatory scaffolding (handled separately at AC-58 - this AC integrates compliance hooks), (e) Moderation (in-house Claude-based, defer third-party APIs). Silence-watcher capability DROPPED permanently per AC-55 Phase 3 finding (commit 8d2e1cd): no industry precedent in workforce-comms vendors + GDPR Art 22 risk (AI auto-replying without human review on automated decisions affecting worker = direct Art 22 violation post-SCHUFA judgment). Capability is dead, not deferred. If business need emerges later, capture as separate AC with explicit Art 22 mitigation design. Principle per Manish Day 30 evening: APATRIS owns the channel - WhatsApp messages can be lost, not audited, not automatable cleanly. APATRIS chat = full control + audit + automation. Type: NEW BUILD (umbrella) - likely reframes AC-40 + AC-43 + AC-44 + AC-46 into coherent system rather than parallel features. Upstream gate: AC-58 compliance scaffolding decisions; Yulia legal-input session. Source: Manish Day 30 evening prompt; Day 31 reshape per 3-phase deep research; refined into 5 categories with industry validation + regulatory integration via AC-58. Phase A audit pending after AC-58 scaffolding decisions made; Cross-build flow-back to EEJ parked as future work per Manish Day 31 architect call: refined APATRIS Communication System (WebSocket+Push hybrid / 3-tier AI+approval UX / preference center) flows back to strengthen EEJ once APATRIS Phase B complete. No priority slot scheduled. Captured here so the cross-build doctrine principle has operational continuity reference. | M3 |
| AC-56 | **MessagingTab nav exposure - operational completion of AC-40 Wave 1** - MessagingTab exists in code at DashboardPage.tsx:323-324, AC-40 Wave 1 receiverId silent-misroute fix shipped Day 30 at v305 in production. But BottomNav.tsx exposes no "messages" tab id across ANY tier (Executive / LegalHead / TechOps / Coordinator / Professional all unable to reach it via primary nav OR More-sheet modules per GATE D31-WORKER-NAV-AUDIT). AC-40 fix is operationally inert until MessagingTab surfaces in worker app navigation. Type: ENHANCE (small Wave 1 build - add nav entry + wire route). Scope: workforce-app bottom nav exposes Messages for relevant tiers (likely T5 Professional primary + T4 Coordinator primary, others via More). NOT a redesign of nav structure. Upstream gate: none - ungated, foundation work. Source: GATE D31-WORKER-NAV-AUDIT operator-eyes finding by Manish Sunday Day 31. | M3 |
| AC-57 | **Worker Help tab build - vision-spec implementation** - Per Worker App Vision Master Prompt (commit e0e0990): "Help" tab is one of 5 worker primary tabs. Features: chat with coordinator/office, voice note support, categorized issue reporting (payroll/accommodation/transport/supervisor/legal/document/other), case ID tracking, multilingual AI assistant. T5 Professional currently has NO Help tab in primary nav OR More-sheet. AC-57 is Phase A audit needed first - significant overlap with AC-40 (Help tab AI assistant per AC-40 audit at commit 1f1100c), AC-46 (worker issue/complaint engine), AC-43 (worker-facing AI), and AC-55 (Communication System umbrella). AC-57 may reframe under AC-55 umbrella post deep research. Type: NEW BUILD. Upstream gate: AC-55 deep research informs scope; AC-44 + AC-46 + AC-43 Phase B architecture decisions feed Help tab implementation. Source: GATE D31-WORKER-NAV-AUDIT + Worker App Vision Master Prompt commit e0e0990; Day 31 architect decision per AC-55 synthesis (commit dd4871e): COLLAPSED into AC-55. Reason: Help tab IS the worker-side UI surface where AC-55 Communication System lives. Standalone AC-57 created scattered scope. AC-55 now includes Help tab as Category B worker-facing surface (AI gen + approval consumer) + Category C preference center entry point + Category E moderation visibility. No separate AC-57 Phase A audit needed. AC-55 Phase B Wave 1+ delivers Help tab as one of the worker-facing surfaces. | COLLAPSED-INTO-AC-55 |
| AC-58 | **EU AI Act + GDPR Art 22 + Kodeks pracy 2026 compliance scaffolding** - HARD DEADLINE 2 August 2026 (EU AI Act high-risk obligations enforce, fines up to €15M or 3% global turnover). Compliance scaffolding required across all APATRIS AI features (AC-43 / AC-44 / AC-45 / AC-46 / AC-55) before further AI feature ship. Scope: (a) GDPR Art 22(1) + 22(3) meaningful human review on automated worker-affecting decisions (post-SCHUFA), (b) EU AI Act Art 6 + Annex III high-risk classification + conformity assessment, (c) EU AI Act Art 26(7) pre-deployment worker + representative notification + acknowledgement record, (d) EU AI Act Art 86 right-to-explanation of decision logic, (e) Posted Workers Directive 2018/957 PIP notification + written worker info per posting (country / duration / currency / benefits / return), (f) Kodeks pracy Jan 27, 2026 amendments electronic form integrity + identifiability + RODO compliance verification for AC-39 Wave 1 leave-request flow already shipped; (g) MessagingTab encryption upgrade - current XOR is legacy not crypto-grade (per AC-43 audit Day 30 + AC-55 Phase 1 audit commit 3bec12e Surface 1 evidence). GDPR exposure on worker message privacy. AC-58 Phase A audit decides crypto approach (libsodium / Web Crypto API / E2E vs server-side at-rest). Escalated from AC-55 Category A transport spine to AC-58 compliance scaffolding because crypto-grade encryption is regulatory requirement, not transport optimization. Type: NEW BUILD - regulatory scaffolding. Upstream gate: Yulia legal-input session (now 9 items - AC-50 + AC-39 cutoffs + AC-41 no-show penalty + AC-43 consent text + AC-43 sensitive messages + AC-42 penalty rules + AC-49 ClientContact consent + AC-45 Tier 3 approval + AC-58 compliance scaffolding). Phase A audit needed. Source: AC-55 Phase 3 web research commit 8d2e1cd. | URGENT |
