    ...
    <div className="p-4 min-h-full overflow-y-auto pb-24 bg-[#0c0c0e]">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-white">{t("leaveTab.title")}</h2>
        </div>
        <button onClick={() => setShowForm(!showForm)}
          className="px-3 py-1.5 bg-[#C41E18] text-white rounded-xl text-xs font-bold active:scale-95">
          {showForm ? t("leaveTab.cancel") : t("leaveTab.request")}
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4 mb-4">
          <select value={form.leaveType || "annual"} onChange={e => setForm({ ...form, leaveType: e.target.value })}
            ...
            <option value="annual">{t("leaveTab.typeAnnual")}</option><option value="sick">{t("leaveTab.typeSick")}</option><option value="unpaid">{t("leaveTab.typeUnpaid")}</option>
          </select>
          ...
          <input placeholder={t("leaveTab.reasonPlaceholder")} value={form.reason || ""} onChange={e => setForm({ ...form, reason: e.target.value })}
          ...
          <button onClick={() => submitMutation.mutate(form)} disabled={!form.startDate || !form.endDate}
            className="w-full py-2.5 bg-[#C41E18] text-white rounded-xl text-sm font-bold active:scale-[0.98] disabled:opacity-40">{t("leaveTab.submit")}</button>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-white/30"><Calendar className="w-10 h-10 mx-auto mb-2 opacity-30" /><p className="text-sm font-semibold">{t("leaveTab.noRequests")}</p></div>
      ) : (
        <div className="space-y-2">
          {leaves.map(l => {
            const statusLabel = l.status === "approved" ? t("leaveTab.statusApproved")
              : l.status === "rejected" ? t("leaveTab.statusRejected")
              : t("leaveTab.statusPending");
            return (
            <div key={l.id} className={cn("rounded-2xl border p-3.5",
              ...
              <div className="flex items-center justify-between mb-1">
                <p className="text-xs font-bold text-white">{l.leave_type} · {l.days} {t("leaveTab.days")}</p>
