...
const statusStyle = {
  submitted: { icon: AlertCircle,  pill: "bg-blue-50 text-blue-700 border-blue-200",    label: "Submitted" },
  approved:  { icon: CheckCircle2, pill: "bg-emerald-50 text-emerald-700 border-emerald-200", label: "Approved" },
  rejected:  { icon: XCircle,      pill: "bg-red-50 text-red-700 border-red-200",        label: "Rejected" },
};
            ...
            <motion.form key="form" onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="month" value={month} onChange={e => setMonth(e.target.value)}
                  className="w-full h-10 border rounded-xl px-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/30" />
              </div>
              <div>
              ...
              <button type="submit" disabled={loading || !hours}
                className={cn("w-full h-11 rounded-xl text-sm font-bold transition-all",
                  loading || !hours ? "bg-gray-100 text-gray-400" : "bg-amber-500 text-white hover:bg-amber-600"
                )}>
                {loading
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between ml-1">
        <div>
          <h2 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">My Timesheets</h2>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-white rounded-2xl border shadow-sm p-3 text-center">
          <div className="text-xl font-black text-amber-600">{loading ? "…" : entries.length}</div>
          <div className="text-[10px] text-muted-foreground font-medium mt-0.5 leading-tight">Months<br/>Submitted</div>
        </div>
        <div className="bg-white rounded-2xl border shadow-sm p-3 text-center">
          <div className="text-xl font-black text-foreground">{loading ? "…" : totalHours}</div>
          <div className="text-[10px] text-muted-foreground font-medium mt-0.5 leading-tight">Total<br/>Hours</div>
        </div>
        <div className="bg-white rounded-2xl border shadow-sm p-3 text-center">
          <div className="text-xl font-black text-emerald-600">{loading ? "…" : approvedHours}</div>
      ...
      <div className="space-y-3">
        <h2 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Submission History</h2>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border shadow-sm p-8 flex flex-col items-center text-center">
